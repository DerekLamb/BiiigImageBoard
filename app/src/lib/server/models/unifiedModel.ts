...
export const UnifiedModel = {
    ...
    async getOne(id: string) { //used to get one image/group
        const document = await imageCollection.aggregate([ 
            {
                $unionWith: {
    ...
    async getChildren(groupName: string = "", page = 0, limit = 10) { //want to avoid using this
