<script lang="ts">
	...
	let viewMode = $derived(page.url.searchParams.get('mode') || 'browse') as viewModeType;
	...
	async function confirmBulkDelete() {
		isDeleting = true;
		const imageIds = Array.from(selectedImages);
		const imagesToDelete = data.documents.filter(
			(doc: AppContent) => imageIds.includes(doc._id) && !('groupType' in doc)
		);
...
<div class="midContainer">
	...
	<div class="imageContainer">
		...
		<ControlBar mode={viewMode} {groupMode} {selectionCount} />
	...
	{#if showDeleteModal}
		<div class="modal-overlay" onclick={() => (showDeleteModal = false)}>
			<div class="modal-content" onclick={(e) => e.stopPropagation()}>
				...
				<div class="modal-actions">
					<button onclick={confirmBulkDelete} disabled={isDeleting} class="delete-btn">
